...
necessary folders in the path to the below folders (you may replace the "~"). 
...
## Create Seurat Object
```{r} 
bone_marrow <- CreateSeuratObject(counts = bone_marrow_data,
                                  project = "bonemarrow",
                                  min.cells = 3,
                                  min.features = 200) 
```
...
## Scale Data
```{r} 
bone_marrow <- ScaleData(bone_marrow,
                         features = rownames(bone_marrow)) 
```

## Run PCA
```{r pca} 
bone_marrow <- RunPCA(bone_marrow,
                      features = VariableFeatures(object = bone_marrow)) 
#### Elbow Plot #### 
...
```{r} 
bone_marrow_markers <- FindAllMarkers(bone_marrow,
                                      only.pos = TRUE,
                                      logfc.threshold = 0.25)
